...
class TrustController extends Controller
{
  ...
  public function storetrust($route)
  {
    ...
    if (!empty($_POST) && count($_POST) > 0) {
      if (!empty($_POST['loan_id']) && is_numeric($_POST['loan_id'])) {
        ...
        $loanDetailsArray = $this->loanObject->getOne("zl_id = '{$loanId}'");
        if (empty($loanDetailsArray)) {
          $errorArray[] = "Error while creating trust details. Please try again later...1";
        }
      } else {
        $errorArray[] = "Error while creating trust details. Please try again later...2";
      }
      ...
      if (empty($errorArray)) {
        ...
        //set defaults        
        $post_data['zlt_id_loan_id'] = $loanId;
        $post_data['zlt_user_id'] = $_SESSION['auth']['user_id'];
        $post_data['zlt_created_by'] = $_SESSION['auth']['user_id'];


        //db insert process
        $this->TrustModel = new TrustModel();
        Model::$db->startTransaction();
        $trustResult =  $this->TrustModel->insert($post_data);

        if (is_int($trustResult) && $trustResult > 0) {
          ...
          Model::$db->rollBack();
          $errorArray[] = "Error while creating trust details. Please try again later...3";
        }
        ...
        if ($successFlag) {
          ...
          if (isset($loanDetailsArray['zl_loan_required']) && $loanDetailsArray['zl_loan_required'] == 1) {
            ...
            $_SESSION['proposal']['flash'] = [
              ...
              'message' => 'Trust details saved successfully!'
            ];
          }
      ...
      $errorArray[] = "Error while creating trust details. Please try again later...4";
    }
